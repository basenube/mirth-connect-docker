...
Mirth Connect Server contains the back-end for the management interface and the integration engine component, which performs message filtering, transformation, and transmission.
    ...
    $ docker run -d -P sween/mirth-connect
...
Launch the container and mount the configuration file:
    ...
    $ docker run -d -P -v ~/mirth.properties:/opt/mirth-connect/conf/mirth.properties sween/mirth-connect
...
First, get the name of the running Mirth Connect Server container:
    ...
    bf63b2b30e2c        sween/mirth-connect          "java -jar mirth-ser   26 minutes ago      Up 26 minutes       0.0.0.0:32769->8080/tcp, 0.0.0.0:32768->8443/tcp   fervent_torvalds

Using the name, link the Mirth Connect Server container to the Mirth Shell container:

    $ docker run -it --link fervent_torvalds:mirth-connect sween/mirth-connect java -jar mirth-cli-launcher.jar -a https://mirth-connect:8443 -u admin -p admin
